...
export function VideoPlayer({ src, currentTimeMv, onSecondChange, onDurationChange }) {
    ...
    const renderTime = (time, totalDuration) => {
        if (!timeRef.current)
            return;
    ...
    return (_jsxs("div", { children: [_jsxs("div", { className: "relative grid min-h-[280px] place-items-center overflow-hidden rounded-2xl bg-black lg:min-h-[440px]", children: [src ? _jsx("video", { ref: videoRef, src: src, className: "max-h-[72vh] w-full object-contain", playsInline: true }) : _jsx("p", { className: "text-slate-500", children: "No playback URL available" }), !playing ? (_jsx("button", { className: "focus-ring absolute grid size-[72px] place-items-center rounded-full bg-[var(--primary)]/85 text-white shadow-[0_0_30px_rgba(124,109,250,0.35)]", onClick: togglePlay, children: _jsx(Play, { fill: "currentColor" }) })) : null] }), _jsxs("div", { className: "mt-3 flex h-[52px] items-center gap-3 rounded-xl border border-white/10 bg-[#0b0b16] px-3", children: [_jsx("button", { className: "focus-ring grid size-8 place-items-center rounded-full bg-[var(--primary)] text-white", onClick: togglePlay, children: playing ? _jsx(Pause, { size: 14 }) : _jsx(Play, { size: 14, fill: "currentColor" }) }), _jsxs("span", { ref: timeRef, className: "mono text-xs text-slate-300", children: [formatTime(0), " / ", formatTime(duration)] }), _jsxs("div", { className: "group relative h-2 flex-1 cursor-pointer rounded bg-[#141424]", onClick: seek, children: [_jsx(motion.div, { className: "h-full rounded bg-[var(--primary)]", style: { width: progressWidth } }), _jsx(motion.div, { className: "absolute top-1/2 size-3 -translate-y-1/2 rounded-full border border-white bg-[var(--primary)] opacity-0 transition group-hover:opacity-100", style: { left: progressWidth } })] }), _jsx("button", { className: "focus-ring text-slate-300", onClick: toggleMute, children: muted ? _jsx(VolumeX, { size: 16 }) : _jsx(Volume2, { size: 16 }) }), _jsx("input", { type: "range", min: 0, max: 1, step: 0.01, value: volume, onChange: (event) => changeVolume(Number(event.target.value)), className: "w-16" }), _jsxs("button", { className: "focus-ring mono text-xs text-slate-300", onClick: cycleSpeed, children: [speed, "x"] }), _jsx("button", { className: "focus-ring text-slate-300", onClick: () => videoRef.current?.requestFullscreen(), children: _jsx(Maximize, { size: 15 }) })] })] }));
}
